ent-Cautery = cauterizador
    .desc = Una herramienta quirúrgica usada para cauterizar heridas abiertas.

ent-Drill = taladro
    .desc = Un taladro quirúrgico para hacer agujeros en material duro.

ent-Scalpel = bisturí
    .desc = Una herramienta quirúrgica usada para hacer incisiones en carne.

ent-ScalpelShiv = cuchillo improvisado
    .desc = Un trozo puntiagudo de vidrio, afilado hasta el filo y envuelto en cinta adhesiva para hacer un mango.

ent-ScalpelAdvanced = bisturí avanzado
    .desc = Hecho de materiales más caros, más afilado y generalmente más fiable.

ent-ScalpelLaser = bisturí láser
    .desc = Un bisturí que usa un láser dirigido para cortar en lugar de una hoja, para una cirugía más precisa mientras también cauteriza al cortar.

ent-Retractor = retractor
    .desc = Una herramienta quirúrgica usada para mantener abiertas las incisiones.

ent-Hemostat = hemostato
    .desc = Una herramienta quirúrgica usada para comprimir vasos sanguíneos y prevenir el sangrado.

ent-Saw = sierra de metal
    .desc = Para cortar madera y otros objetos en pedazos. O serrar huesos, en caso de emergencia.

ent-SawImprov = choppa
    .desc = Una malvada hoja serrada hecha de lo que sea punzante y desagradable que hayas podido encontrar.

ent-SawElectric = sierra circular
    .desc = Para corte de alta potencia.

ent-SawAdvanced = sierra circular avanzada
    .desc = Crees que puedes cortar cualquier cosa con ella.
